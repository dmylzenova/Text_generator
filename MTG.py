...
class GenerateText(object):
    ...
    def distribute(cases):
        ...
        rand_value = random.randint(0, total_frequency)
        aggregated_frequency = 0
        for item in cases:
            aggregated_frequency += item[1]
            if rand_value <= aggregated_frequency:
                return item[0]
    ...
    def generate_text(self):
        ...
        while current_length < self.text_length:
            ...
            if current_word != '*' and current_word not in '.?!,;:':
...
def main(text_length):
    statistics = CreateStatistics('rus_input_file.txt')
